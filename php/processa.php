...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($_POST['act'] == "relatorio_mensal"){
        ...
        if(!empty($placa_Carro)){
            Relatorio_mensal_com_carro($data_relatorio,$placa_Carro);
        }else{
            Relatorio_mensal_sem_carro($data_relatorio);
        }
...
function dataAgendamento(){
    ...
    $sql = "SELECT data_reserva,turno FROM reservas";
...
function devolver_carro($status,$nome,$nome_carro,$placa,$data,$km,$turno,$destino){
    ...
    */
    ...
    $sql = "SELECT kilometragem,tipo_agendamento FROM carros WHERE placa = '$placa'";
    $Retorno_consulta;
    $tipo;
    ...
    if ($Retorno_consulta !== null && $Retorno_consulta < $km) {
        $sql = "UPDATE carros SET situacao = 'livre', kilometragem = '$km',tipo_agendamento = null WHERE placa = '$placa'";
    
        if($conn->query($sql) === TRUE) {
            Delete_Registro_Reserva($conn, $placa);
            Insert_Relatorio($condicao, $status, $nome, $nome_carro, $destino, $placa, $data, $km, $turno,$tipo);
        } else {
            $conn->close();
            echo 0;
        }
...
function index(){
    ...
    $sql = "SELECT c.situacao, c.nome,c.tipo_agendamento, c.placa, r.data_saida, r.turno, r.destino FROM carros c LEFT JOIN registros r ON c.placa = r.placa";
    $Retorno_consulta = array();
    $result = mysqli_query($conn,$sql); 
    ...
    $sql2 = "SELECT c.situacao,c.nome,c.tipo_agendamento,c.placa, re.data_reserva, re.turno, re.destino FROM carros c LEFT JOIN reservas re ON c.placa = re.placa";
    ...
    foreach ($Retorno_consulta as $linha) {
        $placa = $linha[3]; // Índice 2 corresponde à coluna da placa
    ...
    foreach ($gruposPlaca as $grupo) {
        ...
        foreach ($grupo as $linha) {
            if ($linha[4] !== null) { // Verifica se a coluna após a placa não é null
...
function tipoAgendamento($conn,$placa,$tipo){
    
    $sql = "UPDATE carros SET tipo_agendamento = '$tipo' WHERE placa = '$placa'";
...
function Imediata($status,$nome,$nome_carro,$destino,$placa,$horario_saida,$km_saida,$turno){
    ...
    if($conn->query($sql) === TRUE){
        ...
        tipoAgendamento($conn,$placa,$tipo);
        Insert_Relatorio($condicao,$status,$nome,$nome_carro,$destino,$placa,$horario_saida,$km_saida,$turno,$tipo);
    }else{
        $conn->close();
        echo 0;
    }
...
function Insert_Relatorio($condicao,$status,$nome,$nome_carro,$destino,$placa,$horario_saida,$km_saida,$turno,$tipo){

    $conn = Get_Conn_sistema_carros();

    $sql = "INSERT INTO relatorio (condicao,status,nome,nome_carro,placa,data,kilometragem,turno,destino,tipo_agendamento) VALUES ('$condicao','$status','$nome','$nome_carro','$placa','$horario_saida','$km_saida','$turno','$destino','$tipo')";
...
function Relatorio_mensal_com_carro($data_relatorio,$placa_Carro){
    ...
    $sql = "SELECT condicao,status, nome, nome_carro, placa, data,kilometragem,turno,destino FROM relatorio WHERE MONTH(data) = $mes AND YEAR(data) = $ano AND placa = '$placa_Carro'";
    $Retorno_consulta = array();
    $result = mysqli_query($conn,$sql);
...
function Relatorio_mensal_sem_carro($data_relatorio){
